...
class StableDiffusionMask(nn.Module):
    def __init__(
        self,
        device,
        fp16=True,
    ...
    @torch.no_grad()
    def get_control_embeds(self, image, name='default', res=256):
        vis_mask = image
        #print(np.max(vis_mask))
        vis_mask[vis_mask[:,:,0] > 0.5,:] = np.array([150, 5, 61])
        #print(vis_mask.shape)
        image = Image.fromarray(np.uint8(vis_mask))
        # image = image.thumbnail((res,res), Image.LANCZOS)
    ...
    def refine(self, pred_rgb, gt_image, mask_image, control_image,
        ...
        ##### prepare control
        control_image[control_image[:,:,0] > 0.5,:] = np.array([150, 5, 61]) / 255.
        control_image = torch.tensor(control_image).to(gt_image)
        control_image = control_image.unsqueeze(0).permute(0,3,1,2)
        control_image = F.interpolate(control_image, (res, res), mode='bilinear', align_corners=False)
        ...
        #control_image = self.control_image['default']
        ...
        for i, t in enumerate(self.scheduler.timesteps[init_step:]):
            ...
            control_image = control_image.to(self.controlnet.dtype)
